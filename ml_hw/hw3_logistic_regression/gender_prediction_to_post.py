...
X_train_extended = np.hstack([X_train_category, X_train_subcategory, X_train_param, X_train_param2, X_train_param3])
X_val_extended = np.hstack([X_val_category, X_val_subcategory, X_val_param, X_val_param2, X_val_param3])
...
val['gender'] = val_predictions.tolist()
val['gender'] = val['gender'].apply(lambda x: 'male' if x == [1] else 'female')

val[['user_id', 'gender']].to_csv('test_predictions.csv', index=False)
